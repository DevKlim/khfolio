document.addEventListener('DOMContentLoaded', function () {
    ...
    const klimentInfo = {
        ...
        heroSubtitle: "AI Engineer & Data Scientist | Crafting Intelligent Solutions & Innovative Experiences.",
        heroVideo: "assets/videos/hero-background-steam.mp4",
        fallbackProjectBannerImage: "assets/images/placeholders/featured_banner_placeholder.jpg"
    };
    ...
    updateTextContent('.hero-content p.hero-subheading', klimentInfo.heroSubtitle);
    if (document.getElementById('hero-video-bg')) {
        const videoSource = document.getElementById('hero-video-bg').querySelector('source');
    ...
    updateTextContent('footer .container p:first-child', `© ${new Date().getFullYear()} ${klimentInfo.name}. All Rights Reserved.`);
    ...
    if (document.body.classList.contains('resume-page')) {
        updateTextContent('.resume-contact-info p a[href^="tel:"]', klimentInfo.phone);
        updateTextContent('.resume-contact-info p a[href^="mailto:"]', klimentInfo.email);
        updateHref('.resume-contact-info p a[href*="linkedin.com"]', klimentInfo.linkedin);
        updateHref('.resume-contact-info p a[href*="github.com"]', klimentInfo.githubProfile); // Uses primary for resume
        ...
        if (resumeFooterYear) resumeFooterYear.textContent = new Date().getFullYear();
    }
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function (e) {
            ...
            if ((window.location.pathname !== '/' && window.location.pathname !== '/index.html') && targetIdClean.startsWith('#')) {
    ...
    function changeNav() {
        ...
        if (window.scrollY < (sections.length > 0 ? sections[0].offsetTop - headerHeight - 50 : window.innerHeight / 2) && (window.location.pathname === '/' || window.location.pathname === '/index.html')) {
        ...
        mainNavLinks.forEach(link => {
            ...
            let sectionIdFromLink = linkHref.includes('#') ? linkHref.substring(linkHref.lastIndexOf('#') + 1) : null;
            ...
            if (sectionIdFromLink === 'featured-curated-projects' && (currentSectionId === 'featured-curated-projects' || currentSectionId === 'all-github-projects' || currentSectionId === 'page-content-start')) {
                link.classList.add('active');
            }
        });
        ...
        if (window.location.pathname.includes('resume.html')) {
            document.querySelector('.main-nav ul li a[href="resume.html"]')?.classList.add('active');
        } else if (window.location.pathname.includes('project-detail.html')) {
            // For project detail, we might want "Projects" to be active
            document.querySelector('.main-nav ul li a[href="#featured-curated-projects"]')?.classList.add('active');
        } else if ((window.location.pathname === '/' || window.location.pathname === '/index.html') && !currentSectionId) {
    ...
    const featuredCuratedSlidesContainer = document.getElementById('featured-curated-slides-container');
    const featuredCuratedDotsContainer = document.getElementById('featured-curated-dots-container');
    ...
    function setupFeaturedCuratedRotator() {
        if (!featuredCuratedSlidesContainer || typeof projectsData === 'undefined' || projectsData.length === 0) {
        ...
        projectsData.forEach((project, index) => {
            ...
            const imageUrl = project.mainImageUrl || project.thumbnailUrl || klimentInfo.fallbackProjectBannerImage;
    ...
    async function fetchGitHubProjects() {
        if (!klimentInfo.githubUsernames || klimentInfo.githubUsernames.length === 0) {
            if (githubProjectsGrid) githubProjectsGrid.innerHTML = '<p>GitHub usernames not configured.</p>';
            if (rotatorContentArea) rotatorContentArea.innerHTML = '<p>GitHub usernames not configured.</p>';
            return;
        }
        ...
        const fetchPromises = klimentInfo.githubUsernames.map(username =>
            fetch(`https://api.github.com/users/${username}/repos?sort=pushed&per_page=100`)
                ...
                .catch(error => {
                    console.warn(`Failed to fetch GitHub projects for ${username}:`, error);
        ...
        try {
            ...
            for (const item of combinedRepos) {
                if (!map.has(item.id)) { // Assuming 'id' is the unique identifier for a repo
        ...
        } catch (error) { // This catch is more for Promise.all itself, though individual errors are handled above.
            console.error("Overall error fetching GitHub projects:", error);
            if (githubProjectsGrid) githubProjectsGrid.innerHTML = `<p class="error-message">Could not load GitHub projects. ${error.message || 'Network error'}</p>`;
            if (rotatorContentArea) rotatorContentArea.innerHTML = `<p class="error-message">Could not load recent projects. ${error.message || 'Network error'}</p>`;
        }
    }
    ...
    function displayGitHubProjects() {
        ...
        if (allGitHubRepos.length === 0) {
            githubProjectsGrid.innerHTML = '<p>No public repositories found or failed to load.</p>';
            return;
        }
                ...
                <a href="${repo.html_url}" target="_blank" class="project-card-link">
                    <div class="project-card">
                        <div class="project-thumbnail placeholder">
                            <span>${repo.name.substring(0, 1).toUpperCase()}</span> 
                        </div>
                        <div class="project-info">
                            ...
                            <div class="project-tags">
                                ...
                                ${repo.topics.slice(0, 2).map(topic => `<span>${topic.charAt(0).toUpperCase() + topic.slice(1)}</span>`).join('')}
                            </div>
                            <div class="repo-stats">
                                <span><i class="fas fa-star"></i> ${repo.stargazers_count}</span>
                                <span><i class="fas fa-code-branch"></i> ${repo.forks_count}</span>
                                ${repo.watchers_count > 0 ? `<span><i class="fas fa-eye"></i> ${repo.watchers_count}</span>` : ''}
                            </div>
                        </div>
                    </div>
                </a>
    ...
    function setupGitHubSidebarRotator() {
        ...
        recentRepos.forEach((repo, index) => {
            const rotatorItemHTML = `
                <div class="rotator-item ${index === 0 ? 'active' : ''}" data-index="${index}">
                    ${repo.owner && repo.owner.avatar_url ? `<img src="${repo.owner.avatar_url}" alt="${repo.owner.login}" class="repo-owner-avatar">` : ''}
                    <h3>${repo.name}</h3>
                    <p>${(repo.description || 'No description.').substring(0, 80)}${(repo.description && repo.description.length > 80) ? '...' : ''}</p>
                    <a href="${repo.html_url}" target="_blank" class="rotator-item-link">View on GitHub &rarr;</a>
                </div>
            `;
        ...
        const prevButton = document.getElementById('rotator-prev');
        const nextButton = document.getElementById('rotator-next');
        if (prevButton) prevButton.style.display = sidebarRotatorItems.length > 1 ? 'inline-block' : 'none';
        if (nextButton) nextButton.style.display = sidebarRotatorItems.length > 1 ? 'inline-block' : 'none';
        ...
        function nextGitHubSidebarItem() {
            let newIndex = (currentGitHubRotatorIndex + 1) % sidebarRotatorItems.length;
            showGitHubSidebarItem(newIndex);
        }
        function prevGitHubSidebarItem() {
            let newIndex = (currentGitHubRotatorIndex - 1 + sidebarRotatorItems.length) % sidebarRotatorItems.length;
            showGitHubSidebarItem(newIndex);
        }
        ...
        if (sidebarRotatorItems.length > 0) {
            ...
            rotatorContentArea.innerHTML = '<p>No recent projects to display in rotator.</p>';
            if (prevButton) prevButton.style.display = 'none';
            if (nextButton) nextButton.style.display = 'none';
        }
    }
    ...
    if (projectDetailSection && typeof projectsData !== 'undefined') {
        ...
        if (project) {
            ...
            updateTextContent('.project-detail-header .tech-stack-display', `Technologies: ${project.techStack.join(', ')}`);

            const mainMediaContainer = projectDetailSection.querySelector('.main-project-image');
            if (mainMediaContainer) {
                mainMediaContainer.innerHTML = `<img src="${project.mainImageUrl || project.thumbnailUrl || klimentInfo.fallbackProjectBannerImage}" alt="${project.title} Main Image">`;
            }

            const galleryContainer = projectDetailSection.querySelector('.project-image-gallery');
            if (galleryContainer) {
            ...
            const descriptionPanel = projectDetailSection.querySelector('.project-description-panel .description-content');
            if (descriptionPanel) {
            ...
            if (linksBox) {
                ...
                if (!project.githubUrl && !project.liveDemoUrl) {
                    linksBox.innerHTML = '<p>No external links available for this project.</p>';
                }
            }
        } else {
    ...
    window.openImageModal = function (src) {
        ...
        if (!modal) {
            ...
            modal.innerHTML = '<span class="close-modal" onclick="closeImageModal(event)">&times;</span><img class="modal-content" id="modalImage" alt="Expanded Image">';
